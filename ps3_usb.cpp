...
EP_RECORD ps3_ep_record[ PS3_NUM_EP ];  //endpoint record structure for the PS3 controller
...
void PS3_USB::PS3_init( void )
{
 ...
 /* Initialize data structures for endpoints of device */
    ps3_ep_record[ CONTROL_PIPE ] = *( Usb.getDevTableEntry( 0,0 ));  //copy endpoint 0 parameters
    ps3_ep_record[ OUTPUT_PIPE ].epAddr = 0x02;    // PS3 output endpoint
    ps3_ep_record[ OUTPUT_PIPE ].Attr  = EP_INTERRUPT;
    ps3_ep_record[ OUTPUT_PIPE ].MaxPktSize = EP_MAXPKTSIZE;
    ps3_ep_record[ OUTPUT_PIPE ].Interval  = EP_POLL;
    ps3_ep_record[ OUTPUT_PIPE ].sndToggle = bmSNDTOG0;
    ps3_ep_record[ OUTPUT_PIPE ].rcvToggle = bmRCVTOG0;
    ps3_ep_record[ INPUT_PIPE ].epAddr = 0x01;    // PS3 report endpoint
    ps3_ep_record[ INPUT_PIPE ].Attr  = EP_INTERRUPT;
    ps3_ep_record[ INPUT_PIPE ].MaxPktSize = EP_MAXPKTSIZE;
    ps3_ep_record[ INPUT_PIPE ].Interval  = EP_POLL;
    ps3_ep_record[ INPUT_PIPE ].sndToggle = bmSNDTOG0;
    ps3_ep_record[ INPUT_PIPE ].rcvToggle = bmRCVTOG0;
    
    Usb.setDevTableEntry( PS3_ADDR, ps3_ep_record );              //plug kbd.endpoint parameters to devtable
    delay(200); // give time for address change
    /* read the device descriptor and check VID and PID*/
    rcode = Usb.getDevDescr( PS3_ADDR, ps3_ep_record[ CONTROL_PIPE ].epAddr, DEV_DESCR_LEN , buf );
    if( rcode ) return;
    ...
    /* Configure device */
    rcode = Usb.setConf( PS3_ADDR, ps3_ep_record[ CONTROL_PIPE ].epAddr, PS3_CONFIGURATION );                    
    if( rcode ) return;
    ...
    rcode = Usb.setReport( PS3_ADDR, ps3_ep_record[ CONTROL_PIPE ].epAddr, PS3_F4_REPORT_LEN,  PS3_IF, HID_REPORT_FEATURE, PS3_F4_REPORT_ID , buf );
    if( rcode ) return;
    
    /* Set the PS3 controller LED 1 On */
    for (i=0; i < PS3_01_REPORT_LEN; i++) buf[i] = pgm_read_byte_near( output_01_report + i); 
    rcode = Usb.setReport( PS3_ADDR, ps3_ep_record[ CONTROL_PIPE ].epAddr, PS3_01_REPORT_LEN,  PS3_IF, HID_REPORT_OUTPUT, PS3_01_REPORT_ID , buf );
    if( rcode ) return;
...
void PS3_USB::PS3_poll( void )
{
 ...
 byte rcode = 0;     //return code
    /* poll PS3 */
    rcode = Usb.inTransfer(PS3_ADDR, ps3_ep_record[ INPUT_PIPE ].epAddr, PS3_01_REPORT_LEN, (char *) &report );
    if( rcode )  return;
...
/* default buffer values */
      ...
      Usb.setReport( PS3_ADDR, ps3_ep_record[ CONTROL_PIPE ].epAddr, PS3_01_REPORT_LEN,  PS3_IF, HID_REPORT_OUTPUT, PS3_01_REPORT_ID , buf );
...
void PS3_USB::setBDADDR(unsigned char * bdaddr){
    ...
    Usb.setReport( PS3_ADDR, ps3_ep_record[ CONTROL_PIPE ].epAddr, PS3_F5_REPORT_LEN,  PS3_IF, HID_REPORT_FEATURE, PS3_F5_REPORT_ID , buf ); 
	return;
}

void PS3_USB::getBDADDR(unsigned char * bdaddr){
	char buf[ PS3_F5_REPORT_LEN ];
	Usb.getReport( PS3_ADDR, ps3_ep_record[ CONTROL_PIPE ].epAddr, PS3_F5_REPORT_LEN,  PS3_IF, HID_REPORT_FEATURE, PS3_F5_REPORT_ID , buf );
    for( int i=0; i < 6; i++){
        bdaddr[i] = buf[i + 2];
    }
	return;
